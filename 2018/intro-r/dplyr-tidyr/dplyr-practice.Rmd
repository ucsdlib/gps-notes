...
if we wantedt to move forward with onoy a few of the variables (columns) in our dataframe we could use the select() function. 
...
for the `wide` format:
1. each row is often a site/subject/or patient and you have multiple observation variables containing the same type of data 
* these can be either repeated observations over time, or observations of multiple variables (or a mix of both)

* you man find data input may be simpler or some other applications may prefer the `wide` format. 
...
![longformat](http://swcarpentry.github.io/r-novice-gapminder/fig/14-tidyr-fig4.png)
