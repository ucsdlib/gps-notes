...
# Write code below:
myvector <- 1:26
myvectortimestwo <- myvector * 2
names(myvector) <- LETTERS
myvector
...
```{r eval=FALSE}
# load the data set
cats <- read.csv(file="data/feline-data.csv") # edit the path in the read.csv function to the data file on your computer.
```
...
cats <- read.csv(file="data/feline-data.csv", stringsAsFactors = FALSE) # "stringsAsFactors = FALSE" needs to be added when loading the file
str(cats$coat) # shows `cats` is actually a character vector
...
```{r}
# Write code below:
mymatrix <- matrix(1:50, ncol=5, nrow=10)
mymatrix
```

```{r}
mymatrix <- matrix(1:50, ncol=5, nrow=10, byrow = TRUE) # to fill by row
mymatrix
```
