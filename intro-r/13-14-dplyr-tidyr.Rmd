...
```{r}
setwd('~/workshops/lecture-notes/gps-skills-2017/intro-r/')
...
# Make the plot
ggplot(data = az.countries, aes(x = year, y = gdpPercap, color = continent)) +
  geom_line() + facet_wrap( ~ country)
```

* This code makes the right plot but it also creates some variables (starts.with and az.countries) that we might not have any other uses for
* Just like we used ` %>% ` to pipe data along a chain of dplyr function we can use it to pass data to `ggplot()`. 
* because ` %>% ` uses replaces the first arg. in a functions we don't need to specify the data= argument 
* let's see this works with the above graph
...
* using dplyr lets us simplifies things, for ex. we could combine the first two steps:
...
* I usually prefer this intermediate format in most cases good despite not having ALL observations in 1 column given that all 3 observation variables have different units.
...
* many functions in r, which are often vector based, you usually do not want to do mathematical operations on values with different units.
...
* in fact, these are compatible and you can use a mix of tidyr and dplyr functions together
...
* gather also allows alternate syntax of using the `-` symbol to identify which variables are not to be gathered (i.e. ID variables)
...
* this may seem trivial with this dataset, but sometimes you have 1 ID variable and 40 Observations variables with irregular variable names
...
* we can then spread `gap_long()` to the original intermediate format or the widest format
...
all.equal(gap_normal,gapminder)
```
* great we've gone from longest format back to the intermediate and didn't introduce any errors in our code
* now let's cover the long all the way back to the wide
...
* we need appropriate labels for all our new variables (time*metric combinations)
